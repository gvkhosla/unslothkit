...
def cmd_new(args: argparse.Namespace) -> int:
    ...
    print("  finetunekit data check data/train.jsonl")
...
def cmd_demo(args: argparse.Namespace) -> int:
    ...
    print(f"🦥 Creating FineTuneKit demo at {project_path}")
...
def run_project_script(project: str, script: str, extra: list[str]) -> int:
    ...
    if not (path / script).exists():
        print(f"❌ {path / script} not found. Run `finetunekit new` first?", file=sys.stderr)
        return 1
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="finetunekit", description="Beginner-friendly Unsloth fine-tuning CLI")
    ...
    demo.add_argument("path", nargs="?", default="finetunekit-demo")
