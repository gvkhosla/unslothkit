...
async function runFineTuneKit(args: string[], cwd = REPO): Promise<RunResult> {
  try {
...
export default function fineTuneKitExtension(pi: ExtensionAPI) {
  ...
  pi.registerCommand("finetune", {
    description: "Open the FineTuneKit beginner fine-tuning launcher",
